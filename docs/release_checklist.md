...
- [x] Unit and component suite passes: 731 tests.
- [x] Mock API contract suite passes: 97 tests, and its coverage check exercises all 78 method-and-path rows of contract §5.
- [x] Mobile Playwright suite passes: 26 journeys in Chrome at the primary `390 x 844 px` viewport.
...
- [x] The journeys assert no mock identifier, and Playwright starts and resets the mock API only when `VITE_API_BASE_URL` is unset or names it (README "Running against another API"). Against the mock they start their own app and mock on ports 5174 and 3101 and never reuse a running server, so a `pnpm dev:all` left open cannot feed them stale data (`journeyServers` in `tests/e2e/support/api-target.ts`).
