...
import { availabilityStatus, type ProductSummary } from '../../domain/marketplace.model'

const props = defineProps<{ product: ProductSummary }>()

const stock = computed(() => availabilityStatus(props.product.availability))
</script>

<template>
  <RouterLink
    class="product-link"
    :to="{ name: ROUTE_NAMES.productDetail, params: { productId: product.id } }"
  >
